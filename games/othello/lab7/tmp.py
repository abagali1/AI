...
CORNER_NEIGHBORS_MASK = 0x42c300000000c342
CORNERS = {0, 7, 56, 63}
MASKS = {
    -1: RIGHT_MASK,
    1: LEFT_MASK,
...
def coin_heuristic(board, piece, move=False): # MAX: 100 MIN: -100
    placed = place(board,piece,move) if move else board
    num_player = hamming_weight(placed[piece])
    num_opp = hamming_weight(placed[1^piece])
    return (num_player-num_opp)*20


def mobility_heuristic(board, piece, move=False): # MAX: 0 MIN: -340
    placed = place(board, piece, move) if move else board
...
def next_to_corner(board, piece, move=False): # MAX: 150 MIN: -100000
    placed = place(board, piece, move) if move else board
...
def stable_edge(board, piece, move=False):
    placed = place(board, piece, move) if move else board
    h = 0
    bs = binary_to_board(placed)
    token = 'X' if piece else 'O'
    for edge in EDGES:
        if move and move not in EDGES[edge]:
            continue
...
def evaluate_move(board, move, piece, empty):
    ...
    h = strat(board, piece, move=actual_move)
    h += next_to_corner(board, piece, move=move)
    h += stable_edge(board, piece, move=move)

    return h


def evaluate_board(board, piece, empty):
    strat = coin_heuristic if empty <= 8 else mobility_heuristic

    return strat(board, piece) + stable_edge(board,piece)
...
def heuristic(board, current, opponent, current_moves, current_length, opponent_moves, opponent_length, empty):
    h = evaluate_board(board,current,empty)
