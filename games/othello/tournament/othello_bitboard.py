...
def minimax(board, piece, depth, alpha, beta, possible=[]):
    ...
    if not possible:
        ...
        if state is True or depth == 0:
            return hamming_weight(board[1]) - hamming_weight(board[0]), 0
        else:
    ...
    if piece:
        ...
        for i in current_moves:
            tmp, move = minimax(place(board, piece, MOVES[i]), not piece, depth - 1, alpha, beta)
            if tmp > max_move:
                max_move, best_move = tmp, i
            alpha = max(max_move, alpha)
            if beta <= alpha:
                break
        TREE_CACHE[(board[0], board[1], piece)] = (max_move, best_move)
        return TREE_CACHE[(board[0], board[1], piece)]
    else:
        min_move = 100
        for i in current_moves:
            tmp, move = minimax(place(board, piece, MOVES[i]), not piece, depth - 1, alpha, beta)
            if tmp < min_move:
                min_move, best_move = tmp, i
            beta = min(min_move, beta)
            if beta <= alpha:
                break
        TREE_CACHE[(board[0], board[1], piece)] = (min_move, best_move)
        return TREE_CACHE[(board[0], board[1], piece)]
