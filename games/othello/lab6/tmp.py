...
def pvs(board, depth, alpha, beta, piece):
    state = game_over(board, piece)
    if depth == 0 or state is True:
        return hamming_weight(board[piece]) - hamming_weight(board[not piece])
    current_moves, length = state
    if length == 0:
        val = pvs(board, depth, alpha, beta, not piece)
        return [val[0], val[1:]+[-1]]
    for pos, move in enumerate(current_moves):
        if pos == 0:
            val = pvs(place(board,piece, ), depth-1, -beta, -alpha, not piece)
