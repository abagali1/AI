...
def first_last6(nums):
    return int(nums[0]) == 6 or int(nums[-1]) == 6
def same_first_last(nums):
    return len(nums) >= 1 and nums[0] == nums[-1]
def make_pi(n):
    return [c for c in "31415926535"[:n]]
def common_end(a, b):
...
def sum2(nums):
  return nums[0] + nums[1] if len(nums) >= 2 else 0 if len(nums) == 0 else nums[0]
def middle_way(a, b):
    return [a[1], b[1]]
def make_ends(nums):
...
def caught_speeding(speed, is_birthday):
  ...
  return 2 if 81<=speed else 0 if speed<61 else 1
def sorta_sum(a, b):
