...
# sub_pzls = [[0, 1, 2, 9, 18, 27, 28, 37, 38], [3, 4, 5, 6, 7, 11, 12, 16, 21], [8, 13, 14, 15, 17, 22, 24, 25, 26],
#             [10, 19, 20, 29, 30, 39, 40, 41, 48],
#             [23, 31, 32, 33, 42, 43, 49, 50, 51], [34, 35, 44, 52, 53, 59, 60, 61, 70],
#             [36, 45, 46, 54, 63, 72, 73, 65, 74],
#             [47, 55, 56, 57, 64, 66, 75, 76, 77], [58, 62, 67, 68, 69, 71, 78, 79, 80]]
sub_pzls = [[0,1,2,9,10,11,18,27,28], [3,12,13,14,23,24,25,34,35], [4,5,6,7,8,15,16,17,26],
            [19,20,21,22,29,36,37,38,39], [30,31,32,33,40,47,48,49,50], [41,42,43,44,51,58,59,60,61],
            [52,53,62,69,70,71,78,79,80], [45,46,55,56,57,66,67,68,77], [54,63,64,65,72,73,74,75,76]]

def am_i_bad_at_typing():
    nums = [j for i in sub_pzls for j in i]
    if len(nums) < 81:
        print("NOT ALL INDICIES VISITED")
    elif len(nums) > 81:
...
def make_ct():
    ...
    for i in range(dim):
        for j in range(dim):
            ...
            r = index // dim
            c = index % dim
            s = [pos for pos, elem in enumerate(sub_pzls) if index in elem][0]
            c_t[index] = (r, c, s)
    return c_t
...
def brute_force(pzl: str) -> str:
    ...
    for choice in set_of_choices:
        b_f = brute_force(pzl[:choice] + symbol + pzl[choice + 1:] if symbol else pzl[:index] + choice + pzl[
                                                                                                         index + 1:])  # recur on new puzzle
        if b_f:
