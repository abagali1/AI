...
MASKS = {
    -9: 18374403900871474688,   #     NW     N     NE     
    -8: 18446744073709551360,    #     <<9   <<8    <<7
    -7: 9187201950435737344,    #
    -1: 18374403900871474942,    #     W     .     E
    1: 9187201950435737471,     #    <<1          >>1
    7: 71775015237779198,      #
    8: 72057594037927935,       #     SW    S     SE
    9: 35887507618889599       #    >>7   >>8    >>9
}
...
def dumb7fill(fill, empty, direction):
    flood = 0
    while(fill):
        flood |= fill
        fill = empty&(fill << direction*-1) if direction < 0 else empty&(fill>>direction)
    return flood
...
def to_string(b):
    return '\n'.join(
        [''.join(['{:064b}'.format(b)[INDICES_2D[(i, j)]][0] for j in range(8)]) for i in range(8)]).strip().lower()
